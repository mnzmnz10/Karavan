  ...
  return (
    <Panel position="top-center">
      <div className="flex items-center gap-0.5 rounded-lg border border-slate-200 bg-white px-1.5 py-1 shadow-md">
        <span className="px-1.5 text-[11px] font-medium text-slate-400">{count} seÃ§ili</span>
        ...
        <AlignBtn mode="right" title="SaÄŸa hizala" onAlign={alignNodes}><AlignEndVertical size={15} /></AlignBtn>
        <div className="mx-0.5 h-5 w-px bg-slate-200" />
        <AlignBtn mode="top" title="Ãœste hizala" onAlign={alignNodes}><AlignStartHorizontal size={15} /></AlignBtn>
        ...
        <div className="mx-0.5 h-5 w-px bg-slate-200" />
        <button onClick={() => distributeNodes("h")} title="Yatay daÄŸÄ±t" disabled={count < 3}
        ...
        <button onClick={() => distributeNodes("v")} title="Dikey daÄŸÄ±t" disabled={count < 3}
          className="grid h-7 w-7 place-items-center rounded text-slate-600 hover:bg-slate-100 disabled:opacity-30">
